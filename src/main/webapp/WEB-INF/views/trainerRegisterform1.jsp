        	...
        	<!-- 회원 폼 -->
		   	<form:form action="/user/register/user" id="registerform-user" style="display: none" method="post" modelAttribute="addUserForm" >
				...
				<div class="row g-3">
				    ...
				    <div class="col-12">
					    <h5>이메일</h5> 
					        <form:input type="text" class="form-control bg-light border-0" id="user-email" path="email" placeholder="example@gmail.com ('@'를 포함하세요.)" style="height: 55px;"/>
				    ...
				    <div class="col-12"  >
				        <button id="button-register" class="btn btn-dark w-100 py-3" type="submit">가입하기</button>
				    </div>
				</div>
		 	</form:form>
	   ...
	   $('#user-img').on('click', function() {
		...
		$("#user-id").blur(function() {
			if (idJ.test($(this).val())) {
				...
				$('#id_check').text('영어소문자, 4~12글자만 가능합니다!');
		...
		$("#user-email").blur(function() {
			if (mailJ.test($(this).val())) {
				...
				$('#user-email-check').text('이메일을 확인해주세요!');
