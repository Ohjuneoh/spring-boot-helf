...
<body>
	...
	<div class="card-body">
	    <ul class="nav nav-tabs">
	        ...
	        <li class="nav-item">
	            <a class="nav-link active" href="/salary/tab3Content?id=${param.id }">개인 레슨 수당 내역</a>
	        </li>
	        <li class="nav-item">
	...
	<div class="card-body">
    	<form method="get" action="/salary/tab3Content" id="form-pclLesson-search">
				    ...
				    <div class="card" style="background-color: #f0f8ff; border: none; padding: 20px;"> 
					    <div style="display: flex; justify-content: space-between; align-items: center;">
					        <div>
					            ...
					            <!-- 이번 달 개인레슨 수당 총액 계산하기 Start -->
					        	<c:set var="totalLessonPrice" value="0"/>
								<c:if test="${not empty monthlyPclList }">
								    <c:forEach var="pcl" items="${monthlyPclList }">
								    	<c:if test="${pcl.status == 'Y' }">
								       		<c:set var="totalLessonPrice" value="${totalLessonPrice + pcl.lessonPrice}"/>
								        </c:if>
								    </c:forEach>
								</c:if>
								<!-- 이번 달 개인레슨 수당 총액 계산하기 End -->
			...
			<div style="display: flex; justify-content: space-between; align-items: center;">
	   			<h3>개인레슨 수당 내역 <span id="totalRowsSpan">${totalRows}건</span></h3>
	    		<span>
			...
			<c:choose>
				<c:when test="${not empty monthlyPclList }">
					<c:forEach var="pcl" items="${monthlyPclList }">
						<tr>
							<td><fmt:formatDate value="${pcl.lessonDate }" pattern="yyyy-MM-dd"></fmt:formatDate></td>
							<td>${pcl.time }</td>
							<td>${pcl.userName}</td>
							<td>${pcl.membershipName }</td>
							<c:choose>
								<c:when test="${pcl.status == 'W'}">
									<td>
										<span class="badge" style="background-color: #f5f5f5; color: #d1a655;">수업 전</span>
									</td>
								</c:when>
								<c:when test="${pcl.status == 'Y'}">
									<td>
										<span class="badge" style="background-color: #fff5cc; color: #d1a655;">수업 완료</span>
									</td>
								</c:when>
								<c:otherwise>	
									<td>
										<span class="badge" style="background-color: #f5f5f5; color: #d1a655;">수업 미진행</span>
									</td>
								</c:otherwise>
							</c:choose>
							<td><fmt:formatNumber value="${pcl.membershipPrice}" groupingUsed="true"/>원</td>
							<td style="font-weight: bold;"><fmt:formatNumber value="${pcl.lessonPrice}" groupingUsed="true"/>원</td>
							<td><fmt:formatNumber value="${pcl.unitPrice}" groupingUsed="true"/>원</td>
							<td><c:out value="${pcl.pclPct * 100}"/>%</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
						<tr>
							<td colspan="9">수업 이력이 없습니다.</td>
						</tr>
				</c:otherwise>
			</c:choose>
			</tbody>
	               		...
	               		<nav>
	               			<ul class="pagination justify-content-center">
	               				<li class="page-item ${first ? 'disabled' : '' }">
	               					<a href="tab3Content?page=${prePage }" class="page-link" onclick="changePage(event, ${prePage})">이전</a>
	               				</li>
	               				...
	               				<li class="page-item ${last ? 'disabled' : '' }">
	               					<a href="tab3Content?page=${nextPage }" class="page-link" onclick="changePage(event, ${nextPage})">다음</a>
	               				</li>
	               			</ul>
	               		</nav> 
	     		</c:if>
	     	<!-- 페이지네이션 End -->
	     	<div id="tab3Id" data-user-id="${param.id }"></div>
	     	<div id="tab3Page" data-page="${param.page }"></div>
	     	<div id="tab3TotalRows" data-total-rows="${totalRows }"></div>
    	</form>
	</div>
</div>
	<!--개인레슨 수당 내역 form End -->
    ...
    function searchLessonsByMonth(){
    	var selectedMonth = document.getElementById('dateInput').value;
         ...
         document.querySelector('#form-pclLesson-search').submit();
    }
  ...
  function saveData(){
	   ...
	   const totalRows = document.querySelector("#tab3TotalRows").getAttribute("data-total-rows");
	...
	// 개인 수당 내역 최근 5개월 그래프 관련 js 
	// STEP1. model에서 값 js형식으로 변경 
	var pclPay = [];
    var pclMonth = [];

    <c:forEach var="record" items="${graphData}">
        pclPay.push(${record.pclPay});
        pclMonth.push('${record.month}'); // 'yyyy-MM'
    </c:forEach>
